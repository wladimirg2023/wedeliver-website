...
const content:Record<string,{eyebrow:string,title:string,intro:string,body:string[]}>= {
  ...
  careers:{eyebrow:"Careers",title:"Grow with us.",intro:"We’re building food brands people love—and we’re always looking for talented people who want to grow with us.",body:[]},
...
function PartnershipsPage(){
  ...
  return <div className="partnerships-page"><div className="partnership-top"><p className="partnership-types">Corporate Orders · Condominium Partnerships · Bars &amp; Hotels</p><a className="button partnership-contact" href="/contact?subject=partnership">Contact us →</a></div><div className="partner-programs">{programs.map(program=><article className="partner-program" key={program.title}><div className="program-intro"><span className="program-icon" aria-hidden="true">{program.icon}</span><div><h2>{program.title}</h2><p>{program.summary}</p></div></div><ul className="program-benefits">{program.benefits.map(benefit=><li key={benefit}>{benefit}</li>)}</ul></article>)}</div><div className="partner-promises">{[["★","Quality Food","Premium ingredients and great taste"],["◷","Reliable Delivery","On time, every time"],["□","Made for Sharing","Perfect for any occasion"],["◇","Partnership","We grow together"]].map(([icon,title,text])=><div key={title}><span>{icon}</span><p><strong>{title}</strong>{text}</p></div>)}</div><Image className="partnership-end-image" src="/images/partnerships/meeting.png" alt="Office team sharing a My Sushi box during a meeting" width={1400} height={1122}/><a className="button partnership-contact" href="/contact?subject=partnership">Contact us →</a></div>;
}
...
export default async function Page({params,searchParams}:{params:Promise<{slug:string}>,searchParams:Promise<{subject?:string}>}){
  ...
  if(!page)notFound();
  return <main className={`inner ${slug==="brands"?"brands-page":""}`}><p className="eyebrow">{page.eyebrow}</p><h1>{page.title}</h1><p className="lead">{page.intro}</p>{page.body.map(text=><p key={text}>{text}</p>)}{slug==="about"&&<AboutSections/>}{slug==="brands"&&<BrandsPage/>}{slug==="partnerships"&&<PartnershipsPage/>}{slug==="careers"&&<section className="careers-openings"><p className="eyebrow">Open Positions</p><h2>Join our team.</h2><ul>{["Cook","Admin","Business Developer","Manager"].map(role=><li key={role}>{role}</li>)}</ul><a className="button" href="/contact?subject=careers">Send your application →</a></section>}</main>;
}
